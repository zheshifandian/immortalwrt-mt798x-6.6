...
static struct BUS_DEBUG_INFO conn_infra_sleep_protect_dump[] = {
	{"CONN_CFG_CONN_INFRA_AP2CONN_SLP_STATUS_ADDR", CONN_CFG_CONN_INFRA_AP2CONN_SLP_STATUS_ADDR},
	{"CONN_BUS_CR_GALS_AP2CONN_GALS_DBG_ADDR", CONN_BUS_CR_GALS_AP2CONN_GALS_DBG_ADDR},
	{"CONN_CFG_CONN_INFRA_CONN2AP_SLP_STATUS_ADDR", CONN_CFG_CONN_INFRA_CONN2AP_SLP_STATUS_ADDR},
	{"CONN_BUS_CR_GALS_CONN2AP_GALS_DBG_ADDR", CONN_BUS_CR_GALS_CONN2AP_GALS_DBG_ADDR},
	{"CONN_CFG_CONN_INFRA_OFF_BUS_SLP_STATUS_ADDR", CONN_CFG_CONN_INFRA_OFF_BUS_SLP_STATUS_ADDR},
	{"CONN_CFG_CONN_INFRA_ON_BUS_SLP_STATUS_ADDR", CONN_CFG_CONN_INFRA_ON_BUS_SLP_STATUS_ADDR},
	{"CONN_CFG_CONN_INFRA_WF_SLP_STATUS_ADDR", CONN_CFG_CONN_INFRA_WF_SLP_STATUS_ADDR},
};
...
static INT32 chip_dump_mib_info(struct hdev_ctrl *ctrl, RTMP_STRING *arg)
{
	...
	os_zero_mem(btdcr, sizeof(btdcr));
	os_zero_mem(btbcr, sizeof(btbcr));
	os_zero_mem(brocr, sizeof(brocr));
	os_zero_mem(brbcr, sizeof(brbcr));
	os_zero_mem(brdcr, sizeof(brdcr));
...
static VOID chip_show_bcn_info(struct hdev_ctrl *ctrl, UCHAR bandidx)
{
	...
	MAC_IO_READ32(pAd->hdev_ctrl, BN0_WF_ARB_TOP_TQSM0_ADDR + band_offset, &mac_val);
	MTWF_PRINT("ARB_TQSM0=0x%08x\n", mac_val);

	MAC_IO_READ32(pAd->hdev_ctrl, BN0_WF_ARB_TOP_TQSE0_ADDR + band_offset, &mac_val);
	MTWF_PRINT("ARB_TQSE0=0x%08x\n", mac_val);

	MAC_IO_READ32(pAd->hdev_ctrl, BN0_WF_LPON_TOP_T0CR_ADDR + band_offset, &mac_val);
	MTWF_PRINT("LPON_T0CR=0x%08x\n", mac_val);

	MAC_IO_READ32(pAd->hdev_ctrl, BN0_WF_LPON_TOP_T0TPCR_ADDR + band_offset, &mac_val);
	MTWF_PRINT("LPON_T0TPCR =0x%08x\n", mac_val);

	MAC_IO_READ32(pAd->hdev_ctrl, BN0_WF_LPON_TOP_MPTCR0_ADDR + band_offset, &mac_val);
	MTWF_PRINT("LPON_MPTCR0 =0x%08x\n", mac_val);

	MAC_IO_READ32(pAd->hdev_ctrl, BN0_WF_LPON_TOP_MPTCR1_ADDR + band_offset, &mac_val);
	MTWF_PRINT("LPON_MPTCR1 =0x%08x\n", mac_val);

	for (idx = 0; idx < 10; idx++) {
		MAC_IO_READ32(pAd->hdev_ctrl, BN0_WF_ARB_TOP_BFCR_ADDR + band_offset, &mac_val);
		MTWF_PRINT("ARB_BFCR=0x%08x (loop %d)\n", mac_val, idx);
	}
/* Enable HW TX Auto Debug SOP */

	RTMP_IO_READ32(pAd->hdev_ctrl, (WF_PLE_TOP_TO_N9_INT_ADDR), &mac_val);
...
#define MAX_STA_WORD_NUM ((MAX_LEN_OF_MAC_TABLE + 31)/32)
	...
	for (idx = UL_AC_START; idx <= UL_AC_END; idx++) {
		...
		if (is_show) {
			...
			for (sta_line = 0; sta_line < 256; sta_line++) {
				if (sta[sta_line])
					MTWF_PRINT("%d ", sta_line);
			}
...
{
	UINT32 wtblDW6, curWinSize;

	halWtblReadRaw(pAd, wtbl_idx, WTBL_TYPE_LMAC, 6, 1, &wtblDW6);
	/*
	* wtblDW6 -- 32bit
	* bit[31-28] [27-24] [23-20] [19-16] [15-12] [11-8] [7-4] [3-0]
	*     TID7    TID6    TID5    TID4    TID3    TID2   TID1  TID0
	*/
	curWinSize = (wtblDW6 >> (tid * 4)) & 0x0F;

	if (win_size < 0xF && win_size != curWinSize) {
		wtblDW6 &= ~(0x0F << (tid * 4));
		wtblDW6 |= win_size << (tid * 4);
		halWtblWriteRaw(pAd, wtbl_idx, WTBL_TYPE_LMAC, 6, wtblDW6);
	}
...
static VOID chip_show_dma_info(struct hdev_ctrl *ctrl)
{
	...
	if (IS_RBUS_INF(pAd) || IS_PCI_INF(pAd)) {
		...
		dump_dma_tx_ring_info(ctrl, "T16:FWDL", MT_DMA0_T16_RING_BASE);
		dump_dma_tx_ring_info(ctrl, "T17:Cmd (H2WM)", MT_DMA0_T17_RING_BASE);
		dump_dma_tx_ring_info(ctrl, "T18:TXD (H2WA)", MT_DMA0_T18_RING_BASE);
		dump_dma_tx_ring_info(ctrl, "T19:TXD1 (H2WA)", MT_DMA0_T19_RING_BASE);
		if (MT7986_get_rid_value() != DEFAULT_RID)
			dump_dma_tx_ring_info(ctrl, "T19:TXD1_PCI1 (H2WA)", MT_DMA0_T19_PCI1_RING_BASE);
		dump_dma_tx_ring_info(ctrl, "T20:Cmd (H2WA)", MT_DMA0_T20_RING_BASE);
		dump_dma_rx_ring_info(ctrl, "R0:Event (WM2H)", MT_DMA0_R0_RING_BASE);
		dump_dma_rx_ring_info(ctrl, "R1:Event (WA2H)", MT_DMA0_R1_RING_BASE);
		dump_dma_rx_ring_info(ctrl, "R2:TxDone (WA2H)", MT_DMA0_R2_RING_BASE);
		dump_dma_rx_ring_info(ctrl, "R3:TxDone1 (WA2H)", MT_DMA0_R3_RING_BASE);
		if (MT7986_get_rid_value() != DEFAULT_RID)
			dump_dma_rx_ring_info(ctrl, "R3:TxDone1_PCI1 (WA2H)", MT_DMA0_R3_PCI1_RING_BASE);
		dump_dma_rx_ring_info(ctrl, "R4:Data (MAC2H)", MT_DMA0_R4_RING_BASE);
		dump_dma_rx_ring_info(ctrl, "R5:Data1 (MAC2H)",	MT_DMA0_R5_RING_BASE);
		if (MT7986_get_rid_value() != DEFAULT_RID)
			dump_dma_rx_ring_info(ctrl, "R5:Data1_PCI1 (MAC2H)", MT_DMA0_R5_PCI1_RING_BASE);
...
static INT32 chip_check_txv(IN struct hdev_ctrl *ctrl, IN UCHAR *name, IN UINT32 value, UINT8 band_idx)
{
...
INT chip_chk_exception_type(RTMP_ADAPTER *pAd)
{
	UINT32 macVal = 0;
	UCHAR exp_assert_proc_entry_cnt = 0;
	UINT32 g1_exp_counter_addr = 0x02204FFC;
...
static VOID ShowIrqHistory(RTMP_ADAPTER *pAd)
{
	...
	macVal = 0;
	HW_IO_READ32(pAd->hdev_ctrl, 0x022051C0, &macVal);
	...
	HW_IO_READ32(pAd->hdev_ctrl, 0x022051C8, &start);
	...
	HW_IO_READ32(pAd->hdev_ctrl, 0x022051C4, &start);
...
#define WF_MCU_WM_SW_DEF_CR_MSG_TRACE_PTR_ADDR             0x02205188
#define WF_MCU_WM_SW_DEF_CR_MSG_TRACE_NUM_ADDR             0x0220518C
#define WF_MCU_WM_SW_DEF_CR_MSG_TRACE_IDX_ADDR             0x0220518C
...
#define WM_SW_DEF_PROGRAM_TRACE_BASE_ADDR                   0x02205148
#define WM_SW_DEF_PROGRAM_TRACE_TRACE_PTR_ADDR              0x0220514C
#define WM_SW_DEF_PROGRAM_TRACE_TRACE_NUM_ADDR              0x02205150
#define WM_SW_DEF_PROGRAM_TRACE_TRACE_IDX_ADDR              0x02205148
...
static VOID ShowSchduleTrace(RTMP_ADAPTER *pAd)
{
	...
	MemSectionRead(pAd, (UCHAR *)&(task_info_g[0]), length, 0x02202A18);
...
static INT32 chip_show_fw_debg_info(struct _RTMP_ADAPTER *pAd)
{
	...
	HW_IO_READ32(pAd->hdev_ctrl, 0x02202814, &(TaskStart[0]));
	HW_IO_READ32(pAd->hdev_ctrl, 0x02202810, &(TaskEnd[0]));
	HW_IO_READ32(pAd->hdev_ctrl, 0x02202984, &(TaskStart[1]));
	HW_IO_READ32(pAd->hdev_ctrl, 0x02202980, &(TaskEnd[1]));
	...
	for (i = 0 ; i < 2 ; i++) {
		HW_IO_READ32(pAd->hdev_ctrl, 0x022027B8+(i*368), &t1);
		HW_IO_READ32(pAd->hdev_ctrl, 0x022027BC+(i*368), &t2);
		HW_IO_READ32(pAd->hdev_ctrl, 0x022027C4+(i*368), &t3);
	...
	macVal = 0;
	HW_IO_READ32(pAd->hdev_ctrl, 0x022050E0, &macVal);
	MTWF_PRINT("       CPU_ITYPE = 0x%x\n", macVal);

	macVal = 0;
	HW_IO_READ32(pAd->hdev_ctrl, 0x022050E8, &macVal);
	MTWF_PRINT("       CPU_EVA = 0x%x\n", macVal);

	macVal = 0;
	HW_IO_READ32(pAd->hdev_ctrl, 0x022050E4, &macVal);
...
static INT32 chip_show_bus_debug_info(struct _RTMP_ADAPTER *pAd)
{
	UINT32 macVal = 0, i = 0;
	BOOL  AP2CONN_INFRA_ON_OK = TRUE, AP2CONN_INFRA_OFF_OK = TRUE, DUMP_WFSYS = TRUE;

	MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
		"================[AP2CONN_INFRA_ON readable check]==================\n");
	/* no CODA */
	macVal = 0;
	RTMP_IO_READ32(pAd->hdev_ctrl, 0x100030D4, &macVal);
	if (((macVal & 0x00000010) >> 4) == 0 && ((macVal & 0x01000000) >> 24) == 0) {
		MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
			"AP2CONN_INFRA_ON readable check is OK\n");
	} else {
		MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
			"AP2CONN_INFRA_ON readable check is NOT OK, rx=0x%x, tx=0x%x\n",
			(macVal & 0x00000010) >> 4, (macVal & 0x01000000) >> 24);
		AP2CONN_INFRA_ON_OK = FALSE;
	}
	if (AP2CONN_INFRA_ON_OK == TRUE) {
		MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
			"================[AP2CONN_INFRA_OFF readable check]==================\n");
		...
		if (((macVal & CONN_HOST_CSR_TOP_BUS_MCU_STAT_HCLK_FR_CK_DETECT_MASK) >>
			...
			MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
				"Check conn_infra off bus clock is OK\n");
		} else {
			MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
				"Check conn_infra off bus clock is NOT OK, ");
			MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
				"conn_infra off bus clock=0x%x, osc clock=0x%x\n",
		...
		if (macVal == 0x02070000) {
			MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
				"Check conn_infra IP version is OK\n");
		} else {
			MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
				"Check conn_infra IP version is NOT OK, CONN_CFG_IP_VERSION_VAL = 0x%08x\n", macVal);
			AP2CONN_INFRA_OFF_OK = FALSE;
		}
		...
		if (((macVal & CONN_HOST_CSR_TOP_DBG_DUMMY_5_CONN_INFRA_BUS_TIMEOUT_IRQ_B_MASK) >>
			...
			MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
				"Check conn_infra off domain bus hang irq status is OK\n");
		} else {
			MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
				"Check conn_infra off domain bus hang irq status is NOT OK, ");
			MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
				"conn_infra_bus_timeout_irq_b = 0x%x\n",
		...
		if (AP2CONN_INFRA_OFF_OK == FALSE) {
			MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
				"================[Power_Check]==================\n");
			/* Check subsys power status */
			macVal = 0;
			RTMP_IO_READ32(pAd->hdev_ctrl, CONN_HOST_CSR_TOP_CONN_INFRA_SYSSTRAP_DBG_ADDR, &macVal);
			MTWF_DBG_NP(DBG_CAT_MISC, CONN_HOST_CSR_TOP_CONN_INFRA_SYSSTRAP_DBG_ADDR, DBG_LVL_INFO,
				"Dump2Excel[Power Status]: CONN_HOST_CSR_TOP_CONN_INFRA_SYSSTRAP_DBG_ADDR = 0x%08x\n",
				macVal);
			MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
				"================[conn_infra_bus_debug]==================\n");
			...
			for (i = 0; i < sizeof(conn_infra_ahb_apb_timeout_dump) / sizeof(struct BUS_DEBUG_INFO); i++) {
				...
				MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
					"Dump2Excel[ahb_apb_timeout_dump]: No.%d. %s = 0x%08x\n",
			...
			for (i = 0; i < sizeof(conn_infra_debug_log_reg) / sizeof(UINT32); i++) {
				...
				MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
					"Dump2Excel[debug_ctrl_setting][Debug log]: NO.%d. 0x%08x VAL = 0x%08x\n",
			...
			for (i = 0; i < sizeof(conn_infra_real_time_log_set_val) / sizeof(UINT32); i++) {
				...
				RTMP_IO_READ32(pAd->hdev_ctrl, CONN_HOST_CSR_TOP_CONN_INFRA_ON_DEBUG_CTRL_AO2SYS_OUT_ADDR,
					&macVal);
				MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
					"Dump2Excel[debug_ctrl_setting][Real time log]: No.%d. VAL = 0x%08x\n",
					i+1, macVal);
			}
			/* conn_infra_sleep_protect_dump */
			for (i = 0; i < sizeof(conn_infra_sleep_protect_dump) / sizeof(struct BUS_DEBUG_INFO); i++) {
				macVal = 0;
				RTMP_IO_READ32(pAd->hdev_ctrl, conn_infra_sleep_protect_dump[i].reg_address, &macVal);
				MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
					"Dump2Excel[conn_infra_sleep_protect_dump]: No.%d. %s = 0x%08x\n",
					i+1, conn_infra_sleep_protect_dump[i].reg_name, macVal);
			}
		}
		if (DUMP_WFSYS == TRUE) {
			MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
			...
			for (i = 0; i < sizeof(enable_debug_set_val) / sizeof(UINT32); i++) {
				...
				RTMP_IO_READ32(pAd->hdev_ctrl, CONN_HOST_CSR_TOP_WF_MCUSY_VDNR_BUS_DBG_OUT_ADDR,
					&macVal);
				MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
					"Dump2Excel[CR_table]: #%d VALUE = 0x%08x\n", i+1, macVal);
			}
			/* bus_ahb_apb_timeout */
			for (i = 0; i < sizeof(wfsys_bus_ahp_apb_timeout) / sizeof(struct BUS_DEBUG_INFO); i++) {
				...
				MTWF_DBG_NP(DBG_CAT_MISC, DBG_SUBCAT_ALL, DBG_LVL_INFO,
					"Dump2Excel[bus ahb apb timeout]: #%d. %s = 0x%08x\n",
...
static struct {
    ...
    {"ULM1",            0x00900000,     0x00010000},
    {"ULM2",            0x02200000,     0x00050000},
    {"ULM3",            0x02300000,     0x00050000},
    {"SRAM",            0x00400000,     0x00018000},
    {"CRAM",            0xE0000000,     0x00114000},
    {NULL,}
};
...
static INT chip_set_fw_cp_util_en(RTMP_ADAPTER *pAd, UINT en)
{
	...
	if (en > 0) {
		...
		HW_IO_WRITE32(pAd->hdev_ctrl, 0x89010108, 0x00000002);
		HW_IO_WRITE32(pAd->hdev_ctrl, 0x89010118, 0x00000002);
		mdelay(10);
		...
		//DIC_CPU_UTLZ_DIS
		HW_IO_WRITE32(pAd->hdev_ctrl, 0x7C053A50, 0x1302);
		HW_IO_WRITE32(pAd->hdev_ctrl, 0x89010108, 0x00000002);
		HW_IO_WRITE32(pAd->hdev_ctrl, 0x89010118, 0x00000002);
	}
...
VOID mt7986_chip_dbg_init(struct _RTMP_CHIP_DBG *dbg_ops)
{
	...
	dbg_ops->show_bus_dbg_info = chip_show_bus_debug_info;
