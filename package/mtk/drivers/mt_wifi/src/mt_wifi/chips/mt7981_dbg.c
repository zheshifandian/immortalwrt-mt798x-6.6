...
static INT32 chip_show_agg_info(struct hdev_ctrl *ctrl, RTMP_STRING *arg)
{
	...
	UINT32 value, idx, agg_rang_sel[15], ampdu_cnt[11], band_offset = 0, total_ampdu = 0;
	UINT8 band_idx = 0;
...
static INT32 chip_dump_mib_info(struct hdev_ctrl *ctrl, RTMP_STRING *arg)
{
	...
	UINT32 mac_val0, mac_val, mac_val1, idx, band_idx = 0, band_offset = 0;
		...
		*/
		...
		for (idx = 0; idx < bss_nums; idx++) {
			MTWF_PRINT("%d\t 0x%x/0x%x\t 0x%x \t 0x%x/0x%x \t 0x%x\n",
					  idx, btocr[idx], btdcr[idx], btbcr[idx], brocr[idx],
					  brdcr[idx], brbcr[idx]);
		}
		...
		for (idx = 0; idx < bss_nums; idx++) {
			MTWF_PRINT("%d:\t0x%x/0x%x  0x%x \t 0x%x \t  0x%x/0x%x/0x%x\n",
					  idx, mbxsdr[idx][0], mbxsdr[idx][1], mbxsdr[idx][2],
					  mbxsdr[idx][3], mbxsdr[idx][4], mbxsdr[idx][5],
					  mbxsdr[idx][6]);
		}
...
static INT32 chip_show_pse_info(struct hdev_ctrl *ctrl, RTMP_STRING *arg)
{
	...
	UINT32 pse_stat, pg_flow_ctrl[22] = {0};
...
{
	RTMP_ADAPTER *pAd = ctrl->priv;
	UINT32 ple_buf_ctrl, pg_sz, pg_num;
...
{
	UINT32 base, cnt, cidx, didx, queue_cnt;
	...
	struct _ATE_RU_STA *sta_list)
{
	...
	UINT32 cr_value, usr_grp_idx = 0, sta_idx = 0, nss;
	union hetb_rx_cmm cmm;
...
INT chip_chk_exception_type(RTMP_ADAPTER *pAd)
{
	...
	UINT32 g1_exp_counter_addr = 0x02204F68;
...
static VOID ShowIrqHistory(RTMP_ADAPTER *pAd)
{
	...
	macVal = 0;
	HW_IO_READ32(pAd->hdev_ctrl, 0x02205138, &macVal);
	...
	HW_IO_READ32(pAd->hdev_ctrl, 0x02205140, &start);
	...
	HW_IO_READ32(pAd->hdev_ctrl, 0x0220513C, &start);
...
#define WF_MCU_WM_SW_DEF_CR_MSG_TRACE_PTR_ADDR             0x02205100
#define WF_MCU_WM_SW_DEF_CR_MSG_TRACE_NUM_ADDR             0x02205104
#define WF_MCU_WM_SW_DEF_CR_MSG_TRACE_IDX_ADDR             0x02205104
#define WF_MCU_WM_SW_DEF_CR_MSG_INFO_R0_PTR_ADDR           0x00900880
#define WF_MCU_WM_SW_DEF_CR_MSG_INFO_R0_NUM_ADDR           0x00900870

#define MSG_HISTORY_NUM 64
#define MAX_TASK_NUM 2

#define WM_SW_DEF_PROGRAM_TRACE_BASE_ADDR                 0x022050C0
#define WM_SW_DEF_PROGRAM_TRACE_TRACE_PTR_ADDR            0x022050C4
#define WM_SW_DEF_PROGRAM_TRACE_TRACE_NUM_ADDR            0x022050C8
#define WM_SW_DEF_PROGRAM_TRACE_TRACE_IDX_ADDR            0x022050C0
...
static VOID ShowSchduleTrace(RTMP_ADAPTER *pAd)
{
	...
	MemSectionRead(pAd, (UCHAR *)&(task_info_g[0]), length, 0x02202978);

	HW_IO_READ32(pAd->hdev_ctrl, 0x0220512C, &km_total_time);
	if (km_total_time == 0) {
...
static INT32 chip_show_fw_debg_info(struct _RTMP_ADAPTER *pAd)
{
	...
	HW_IO_READ32(pAd->hdev_ctrl, 0x022028B4, &(TaskStart[0]));
	HW_IO_READ32(pAd->hdev_ctrl, 0x022028C0, &(TaskEnd[0]));
	...
	for (i = 0 ; i < 2 ; i++) {
		HW_IO_READ32(pAd->hdev_ctrl, 0x02202718+(i*368), &t1);
		HW_IO_READ32(pAd->hdev_ctrl, 0x0220287C+(i*368), &t2);
		HW_IO_READ32(pAd->hdev_ctrl, 0x02202884+(i*368), &t3);

		if (i == 0) {
			ret = snprintf(str, sizeof(str), "%s", "WMT");
	...
	macVal = 0;
	HW_IO_READ32(pAd->hdev_ctrl, 0x02205058, &macVal);
	MTWF_PRINT("       CPU_ITYPE = 0x%x\n", macVal);

	macVal = 0;
	HW_IO_READ32(pAd->hdev_ctrl, 0x02205060, &macVal);
	MTWF_PRINT("       CPU_EVA = 0x%x\n", macVal);

	macVal = 0;
	HW_IO_READ32(pAd->hdev_ctrl, 0x0220505C, &macVal);
...
static struct {
    ...
    {"_ROM.bin",        0x00800000,     0x00038000},
    {"ULM1",            0x00900000,     0x0000C000},
    {"ULM2",            0x02200000,     0x00050000},
    {"ULM3",            0x02300000,     0x00050000},
    {"SRAM",            0x00400000,     0x00018000},
    {"CRAM",            0xE0000000,     0x00110000},
    {NULL,}
};
